<template>
  <form @submit.prevent="handleSubmit" class="flex flex-col gap-6">
    <IonList class="ion-no-padding">
      <IonItem lines="full" class="mt-4">
        ...
        <IonInput
          ...
          :aria-invalid="nameTouched && localName.trim().length === 0"
          :aria-describedby="nameErrorId"
          @ionBlur="nameTouched = true"
        />
      </IonItem>
      <IonText
        v-if="nameTouched && localName.trim().length === 0"
        color="danger"
        :id="nameErrorId"
      >
    ...
    <IonButton
      ...
      expand="block"
      class="mt-4 px-4"
      :disabled="localName.trim().length === 0 || disabled"
    >
      {{ submitLabel }}
    </IonButton>
  </form>
...
// Props for customizing the form labels, placeholders, and state
...
// Emits the submit event with form data
...
// ID for the name error message (for accessibility)
const nameErrorId = "entity-name-error";

// Handle form submission, validate, and emit data
