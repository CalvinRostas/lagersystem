  ...
  <IonPage>
    ...
    <IonContent :fullscreen="true" class="ion-padding">
      ...
      <div v-else class="flex flex-col gap-6">
        ...
        />
        <QRCodeDisplay
          :qr-code-data-url="item.qrCodeDataUrl"
          :alt-text="`QR code for ${item.name}`"
        />
      </div>
    </IonContent>
  </IonPage>
...
// Access item data and remove function from composable
const { items, remove } = useItems();
// Access storage location lookup from composable
const { getById } = useUseStorageLocation();

// Computed property for the current item based on route param
const item = computed(() => {
  const id =
...
// Computed property for the item's storage location (if any)
const storageLocation = computed(() => {
  if (!item.value?.storageLocationId) return null;
  return getById(item.value.storageLocationId);
});

// Confirm and delete the item, then navigate back to the home page
function confirmDelete() {
  if (!item.value) return;
  if (!window.confirm(`Delete "${item.value.name}"? This cannot be undone.`))
    return;
